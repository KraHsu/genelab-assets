"""mjlab `csv_to_npz` converter — verbatim vendor copy.

Source : https://github.com/Mujoco-Lab/mjlab/blob/main/src/mjlab/scripts/csv_to_npz.py
License: Apache-2.0 (mjlab) — see mjlab's LICENSE.

This file is kept here as a reference snapshot so the NPZ blobs in this
directory can be regenerated without cloning mjlab. To actually run it, install
mjlab (`uv pip install mjlab`) and invoke via `scripts/convert.sh` — the
imports below pull in mjlab + mujoco-warp at runtime.
"""
